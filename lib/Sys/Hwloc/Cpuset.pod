...
Sys::Hwloc::Cpuset is the Perl namespace used for B<struct hwloc_cpuset>
data prior hwloc v1.1.

Since hwloc v1.1, this class is replaced by Sys::Hwloc::Bitmap.
...
every hwloc C function that gets a hwloc_cpuset pointer as first argument
...
ORes $set with $oset, similar to B<|=>.
...
ANDs $set with $oset, similar to B<&=>.
...
XORes $set with $oset, similar to B<^=>.
